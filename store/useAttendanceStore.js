...
export const useAttendanceStore = defineStore('attendance', {
  ...
  getters: {
    ...
    summary: (state) => {
      ...
      const absent = state.attendances.filter(a => a.status === 'absent').length
      const excused = state.attendances.filter(a => a.status === 'excused').length
  ...
  actions: {
    async fetchAttendance() {
      ...
      try {
        ...
        this.attendances = response.data.data.map(item => ({
          ...
          subject_code: item.session?.course_offering?.subject?.subject_code || 'N/A',
          subject_name: item.session?.course_offering?.subject?.subject_name || 'Unknown',
    ...
    async createAttendance(data) {
      try {
        const { $UserPrivateAxios } = useNuxtApp()
        const response = await $UserPrivateAxios.post('/users/attendance/', data)
