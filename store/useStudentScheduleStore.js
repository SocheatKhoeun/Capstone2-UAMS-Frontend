...
export const useStudentScheduleStore = defineStore('studentSchedule', {
  ...
  actions: {
    _client() {
      ...
      const axiosInstance = nuxtApp?.$UserPrivateAxios || nuxtApp?.$axios || null
      const base = (config.public?.USER_PRIVATE_API || config.public?.API_BASE_URL || '').replace(/\/+$/, '')
    ...
    async fetchSchedule() {
      ...
      try {
        ...
        const res = await client.get(`${base}/users/sessions/`)
