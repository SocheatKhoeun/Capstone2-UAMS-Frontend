  ...
  actions: {
    _normalize(term) {
      return {
        ...
        term: term.term ?? '',
        active: term.active === 1 || term.active === true || term.active === '1' ? 1 : 0,
    ...
    async fetchTerms() {
      ...
      try {
        ...
        const res = await client.get(`${base}/terms/`)
    ...
    async updateTerm(globalId, payload) {
      ...
      try {
        ...
        const idx = this.terms.findIndex(term =>
          String(term.id) === String(normalized.id) || String(term.global_id) === String(normalized.global_id)
        )
    ...
    async deleteTerm(globalId) {
      ...
      try {
        ...
        this.terms = this.terms.filter(term =>
          String(term.global_id) !== identifier
        )
        return res
      } catch (err) {
