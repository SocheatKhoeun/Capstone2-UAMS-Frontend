            ...
            <div class="content-container">
                ...
                <div class="toolbar-section">
                    <div class="toolbar-header">
                        ...
                        <v-chip
                            v-if="filteredSchedules.length !== schedules.length"
                            size="small"
                            color="primary"
                            variant="tonal"
                        >
                            {{ filteredSchedules.length }} of
                            {{ schedules.length }}
                        </v-chip>
                    </div>
                        ...
                        >
                            ...
                            <div class="card-footer">
                                ...
                                <v-btn
                                    ...
                                    size="small"
                                    color="primary"
                                    @click.stop="openDetailDialog(schedule)"
                                    class="view-btn"
                                >
                                    View Details
                                    <v-icon end size="16"
                ...
                <div class="detail-dialog-header">
                    <div class="header-content">
                        ...
                        <div class="header-text">
                            ...
                            <p class="detail-dialog-subtitle">
                                Complete schedule information overview
                            </p>
                        </div>
                    </div>
                    <v-btn
                        ...
                        variant="text"
                        size="small"
                        @click="scheduleDetailDialog = false"
                        class="close-btn"
                    />
                </div>
                    ...
                    <div class="detail-info-grid">
                        ...
                        <div class="info-card">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value">
                                    {{ getGroupLabel(selectedSchedule) }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="info-card">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value">
                                    {{ getTermName(selectedSchedule.term_id) }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="info-card">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value">
                                    {{
                                        getGenerationName(
                                            selectedSchedule.generation_id,
                                        )
                                    }}
                                </div>
                            </div>
                        ...
                        <div class="info-card">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value">
                                    {{ getRoomName(selectedSchedule.room_id) }}
                                </div>
                            </div>
                        </div>
                            ...
                            <v-row>
                                ...
                                <v-col cols="12">
                                    <div class="detail-item">
                                        ...
                                        >
                                        <span
                                            class="detail-value description"
                                            >{{
                        ...
                        <div
                            ...
                            v-if="selectedSchedule.description"
                        >
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value description-text">
                                    {{ selectedSchedule.description || "N/A" }}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <v-card-text class="delete-body">
                    <div class="warning-banner">
                        ...
                        <div class="banner-content">
                            <p class="banner-text">
                                You're about to permanently delete the schedule
                                for
                                <strong>{{
                                    scheduleToDelete
                                        ? getGroupLabel(scheduleToDelete)
                                        : "this schedule"
                                }}</strong>
                            </p>
                        </div>
...
const groupsWithSchedule = computed(() => {
    const ids = new Set();
    schedules.value.forEach((s) => {
        if (Array.isArray(s.groups) && s.groups.length) {
            s.groups.forEach((g) => ids.add(g.id));
            return;
        }
        if (Array.isArray(s.group_ids) && s.group_ids.length) {
            s.group_ids.forEach((id) => ids.add(id));
            return;
        }
        if (s.group_id) ids.add(s.group_id);
    });
    return [...ids];
});
...
const filteredSchedules = computed(() => {
    ...
    if (searchQuery.value) {
        ...
        list = list.filter((s) => {
            return (
                getGroupLabel(s).toLowerCase().includes(q) ||
...
const totalPages = computed(() =>
    Math.ceil(filteredSchedules.value.length / itemsPerPage.value),
);
...
const roomOptions = computed(() =>
    rooms.value.map((r) => ({ title: r.room || "No Room", value: r.id })),
);
const generationOptions = computed(() =>
    generations.value.map((g) => ({ title: g.generation, value: g.id })),
);
...
const openCreateDialog = () => {
    ...
    console.log(
        "Dialog opened - SubjectOptions:",
        subjectOptions.value.length,
        "items",
    );
    console.log(
        "Dialog opened - TermOptions:",
        termOptions.value.length,
        "items",
    );
...
const submitForm = async () => {
    ...
    try {
        ...
        const payload = {
            ...
            course_type: formData.course_type || "lecture",
        ...
        let result;
        if (isEdit.value) {
            result = await scheduleStore.updateSchedule(
                formData.global_id,
                payload,
            );
        } else {
...
const confirmDelete = async () => {
    ...
    const result = await scheduleStore.deleteSchedule(
        scheduleToDelete.value.global_id,
    );
...
const getGroupName = (id) =>
    groups.value.find((g) => g.id === id)?.group_name || "Unknown Group";
const getGroupLabel = (schedule) => {
    if (!schedule) return "Unknown Group";
    if (Array.isArray(schedule.groups) && schedule.groups.length) {
        return schedule.groups
            .map((g) => g.name || g.group_name || g.id)
            .join(", ");
    }
    if (Array.isArray(schedule.group_ids) && schedule.group_ids.length) {
        return schedule.group_ids.map((id) => getGroupName(id)).join(", ");
    }
    if (schedule.group_id) return getGroupName(schedule.group_id);
    return "Unknown Group";
};
const getSubjectName = (id) =>
    subjects.value.find((s) => s.id === id)?.name || "Unknown Subject";
const getRoomName = (id) =>
    rooms.value.find((r) => r.id === id)?.room || "No Room";
const getGenerationName = (id) =>
    generations.value.find((g) => g.id === id)?.generation || "N/A";
...
const fetchReferenceData = async () => {
    ...
    try {
        ...
        const [g, s, t, i, r, gen] = await Promise.all([
...
.info-card::before {
    ...
    background: linear-gradient(
        180deg,
        var(--card-color, #3b82f6),
        var(--card-color-dark, #2563eb)
    );
    opacity: 0;
    transition: opacity 0.3s ease;
}
