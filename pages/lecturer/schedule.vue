            ...
            <div class="table-container">
                ...
                <div class="schedule-grid">
                    ...
                    <div class="time-column header-cell">
                        <div class="pa-3 text-center font-weight-bold">
                            Time
                        </div>
                    </div>
                    ...
                    <template v-for="slot in timeSlots" :key="slot">
                        ...
                        >
                            <div class="pa-2">
                                <div
                                    v-for="session in getSessionsForSlot(
                                        day,
                                        slot,
                                    )"
                                    :key="session.id"
...
const getWeekStart = () => {
    ...
    start.setDate(today.getDate() + diff + currentWeek.value * 7);
...
const scheduleItems = computed(() => {
    return weekSessions.value
        .map((session, idx) => {
            const offering = offeringStore.schedules.find(
...
const todaySessions = computed(() => {
    ...
    return scheduleItems.value.filter((s) => s.day === today).length;
});

const thisWeekSessions = computed(() => scheduleItems.value.length);
...
const getSessionsForSlot = (day, time) => {
    return scheduleItems.value.filter((s) => s.day === day && s.time === time);
};
