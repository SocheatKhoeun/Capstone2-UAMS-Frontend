                    ...
                    <div class="title-wrapper">
                        ...
                        <div class="title-content">
                            ...
                            <div class="breadcrumb">
                                ...
                                <v-icon size="16" class="breadcrumb-divider"
                ...
                <div class="action-section">
                    <v-menu offset-y>
                        ...
                        <v-list class="modern-menu">
                            ...
                            >
                                <v-list-item-title
                                    >Export as PDF</v-list-item-title
                                >
                            </v-list-item>
                            <v-list-item
                                class="menu-item"
                            ...
                            >
                                <v-list-item-title
                                    >Export as Excel</v-list-item-title
                                >
                            </v-list-item>
                        </v-list>
                    </v-menu>
                ...
                <div class="table-toolbar">
                    <div class="toolbar-left">
                        ...
                        <div class="table-subtitle">
                            Monitor and manage student attendance across all
                            sessions
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <template v-slot:item.attendance_rate="{ item }">
                            <div class="attendance-rate-cell">
                                <v-chip
                                    :color="
                                        getAttendanceColor(item.attendance_rate)
                                    "
                                    size="small"
                                    variant="flat"
                                    class="rate-chip"
                        ...
                        <template v-slot:item.last_session_date="{ item }">
                            <div class="date-cell">
                                {{ formatTableDate(item.last_session_date) }}
                            </div>
                        </template>
                        ...
                        <template v-slot:item.class_name="{ item }">
                            <div class="class-info">
                                ...
                                <div class="session-name">
                                    {{ item.session_name || "Regular Session" }}
                                </div>
                            </div>
                        </template>
...
import { useLecturerGroupStore } from "~/store/lecturers/groupStore";
...
const overallStats = computed(() => {
    ...
    const present = attendanceRecords.value.filter(
        (r) => r.status === "Present",
    ).length;
    const absent = attendanceRecords.value.filter(
        (r) => r.status === "Absent",
    ).length;
    const late = attendanceRecords.value.filter(
        (r) => r.status === "Late",
    ).length;
    const excused = attendanceRecords.value.filter(
        (r) => r.status === "Excused",
    ).length;

    // Calculate unique classes and students
    const uniqueClasses = new Set(
        attendanceRecords.value.map((r) => r.class_name),
    ).size;
    const uniqueStudents = new Set(
        attendanceRecords.value.map((r) => r.student_id),
    ).size;
...
const filteredClasses = computed(() => {
    ...
    const query = searchQuery.value.toLowerCase();
    return classSummary.value.filter(
        (classItem) =>
...
const normalizeMethod = (method: string | null | undefined) => {
    ...
    if (m === "face") return "Face Recognition";
    if (m === "qr") return "QR Code";
    if (m === "manual") return "Manual";
    return method;
};

const buildAttendanceDisplay = (record: any) => {
    const session = sessionStore.sessions.find(
        (s) => String(s.id) === String(record.session_id),
    );
    ...
    const subject = offering
        ...
        : null;
    const group = offering
        ? groupStore.groups.find(
              (g) => String(g.id) === String(offering.group_id),
          )
        : null;
    const room =
        roomStore.rooms.find(
            (r) => String(r.id) === String(session?.room_id),
        ) ||
        (offering
            ? roomStore.rooms.find(
                  (r) => String(r.id) === String(offering.room_id),
              )
            : null);
    const student = studentStore.students.find(
        (s) => String(s.id) === String(record.student_id),
    );
    ...
    return {
        ...
        student_id: student?.student_code || String(record.student_id || ""),
        student_name: studentName,
        status: normalizeStatus(record.status),
        method: normalizeMethod(record.method),
        checkin_time: record.checkin_time || record.created_at || "",
        checkout_time: record.checkout_time || "",
        class_name: `${subjectCode} - ${subjectName}`.trim(),
        session_name: group?.group_name || "Session",
        class_id: session?.id,
        room: room?.room || "TBA",
...
const fetchAttendanceData = async () => {
    ...
    attendanceRecords.value = list.map(buildAttendanceDisplay);
};
