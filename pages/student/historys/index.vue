    ...
    <div class="modern-header">
      <div class="header-container">
        <div class="title-section">
          <div class="title-wrapper">
            <div class="title-icon">
              <v-icon icon="mdi-history" size="32" color="white" />
            </div>
        ...
        <div class="action-section">
          ...
          <v-btn class="modern-btn add-btn" prepend-icon="mdi-calendar-plus" variant="flat" @click="openNewLeaveDialog">
            Request Leave
          </v-btn>
        </div>
      </div>
      ...
      <div class="table-container">
        ...
        <div class="table-toolbar">
          <div class="toolbar-left">
            <h2 class="table-title">
              <v-icon icon="mdi-calendar-text" size="20" class="mr-2" />
              Leave Requests
            </h2>
            <div class="table-subtitle">Track all your submitted leave requests</div>
          </div>
          ...
          <div class="toolbar-right">
            ...
            <v-btn icon="mdi-filter-variant" variant="outlined" class="filter-btn" @click="showFilters = !showFilters" />
          </div>
        </div>
            ...
            <div class="filters-content">
              <div class="filter-group">
                ...
                >
                  ...
                  <v-date-picker v-model="filters.dateRange" range @update:model-value="onDateRangeChange" />
                </v-menu>
              </div>
            </div>
          </div>
        ...
        <div class="modern-table-wrapper">
          <v-table class="modern-table">
            <thead>
              <tr class="modern-header-row">
                ...
                <th class="modern-header-cell center-align">
                  <div class="header-content">Submitted At</div>
                </th>
                <th class="modern-header-cell center-align">
            ...
            <tbody>
              <tr v-for="(item, index) in paginatedLeaves" :key="item.id" class="modern-table-row">
                ...
                <td class="modern-table-cell">
                  <div class="period-info">
                    <div class="period-dates">{{ formatDate(item.from_date) }} → {{ formatDate(item.to_date) }}</div>
                  </div>
                </td>
                <td class="modern-table-cell">
                  <div class="reason-text">{{ item.reason || 'N/A' }}</div>
                </td>
                <td class="modern-table-cell center-align">
                  <div class="days-badge">{{ item.days }} day(s)</div>
                </td>
                <td class="modern-table-cell center-align">
                  <v-chip :color="statusColor(item.status)" size="small" class="status-chip">
                    <v-icon start size="16">{{ statusIcon(item.status) }}</v-icon>
                    {{ item.status }}
                  </v-chip>
                </td>
                <td class="modern-table-cell center-align">
                  <div class="date-info">{{ formatDateTime(item.created_at) }}</div>
                </td>
                <td class="modern-table-cell center-align">
                  <div class="action-group">
                    <v-btn icon class="action-btn" @click="viewDetail(item)">
          ...
          <div v-if="filteredLeaves.length === 0" class="empty-state">
            <v-icon icon="mdi-calendar-blank" size="64" color="grey-lighten-1" />
            <h3 class="empty-title">No leave requests found</h3>
            <p class="empty-subtitle">
              You haven't submitted any leave requests yet.
            </p>
            <v-btn color="primary" variant="flat" @click="openNewLeaveDialog" class="mt-4">
    ...
    <v-dialog v-model="detailDialog" max-width="600" persistent>
      <v-card class="modern-dialog" elevation="24" v-if="selectedLeave">
        ...
        <div class="dialog-header">
          <div class="header-content">
            <div class="header-icon">
              <v-icon icon="mdi-calendar-text" color="primary" size="24" />
            </div>
            <div class="header-text">
              <h2 class="dialog-title">Leave Request Details</h2>
              <p class="dialog-subtitle">{{ selectedLeave.reason || 'Leave Request' }}</p>
            </div>
          </div>
          <v-btn icon="mdi-close" variant="text" size="small" @click="detailDialog = false" class="close-btn" />
        </div>
          ...
          <div class="detail-section">
            <div class="detail-grid">
              <div class="detail-item">
                ...
                <div class="detail-value">{{ formatDate(selectedLeave.from_date) }} → {{ formatDate(selectedLeave.to_date) }}</div>
              </div>
              <div class="detail-item">
                <div class="detail-label">Days</div>
                <div class="detail-value">{{ selectedLeave.days }} day(s)</div>
              </div>
              <div class="detail-item">
                <div class="detail-label">Status</div>
                <div class="detail-value">
                  <v-chip :color="statusColor(selectedLeave.status)" size="small">
              ...
              <div class="detail-item">
                ...
                <div class="detail-value">{{ formatDateTime(selectedLeave.created_at) }}</div>
              </div>
            </div>
          </div>
        </v-card-text>
        ...
        <v-card-actions class="dialog-actions">
          <v-btn variant="outlined" color="grey-darken-1" @click="detailDialog = false">
            Close
          </v-btn>
        </v-card-actions>
      </v-card>
...
const exportColumns = [
  ...
  { key: 'from_date', header: 'From Date', format: 'date', width: 14 },
  { key: 'to_date', header: 'To Date', format: 'date', width: 14 },
  { key: 'days', header: 'Days', width: 10 },
  { key: 'reason', header: 'Reason', width: 30 },
  { key: 'status', header: 'Status', width: 12 },
  { key: 'created_at', header: 'Submitted At', format: 'datetime', width: 18 }
]
