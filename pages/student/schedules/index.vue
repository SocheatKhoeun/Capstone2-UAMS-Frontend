  ...
  <div class="schedule-page">
        ...
        <div class="title-section">
          <div class="title-wrapper">
            <div class="title-icon">
              <VIcon icon="mdi-calendar-clock" color="white" size="32" />
            </div>
          ...
          <div class="subtitle-text">
            View your upcoming classes by day, week, or term. You can also export or import your schedule.
          </div>
        </div>
        <div class="header-actions">
          <ExportButtons :data="exportData" :columns="exportColumns" filename="Student_Schedule"
            @export-start="onExportStart" @export-complete="onExportComplete" @export-error="onExportError" />
          <ImportCsv :columns="exportColumns" @import-complete="onImportComplete" @import-error="onImportError" />
          <VBtn class="refresh-btn" variant="flat" color="primary" prepend-icon="mdi-refresh" @click="fetchSchedule">
            Refresh
          </VBtn>
        </div>
      </div>
    ...
    <div class="content-section">
      ...
      <VCard class="filter-card" elevation="0">
        <VCardText>
          <div class="filter-controls">
            <VSelect v-model="filters.termId" :items="termItems" item-title="label" item-value="value" label="Term"
              density="comfortable" clearable class="filter-item" />
            <VMenu v-model="dateMenu" :close-on-content-click="false" transition="scale-transition" max-width="290"
              min-width="290">
              <template #activator="{ props }">
                <VTextField v-bind="props" v-model="dateRangeLabel" label="Date range" readonly density="comfortable"
                  class="filter-item" prepend-inner-icon="mdi-calendar-range" />
              </template>
              <VDatePicker v-model="dateRange" range @update:model-value="onDateRangeChange" />
            </VMenu>
            <div class="switch-wrapper">
              <VSwitch v-model="filters.todayOnly" label="Show today only" color="primary" hide-details />
            </div>
            <VBtn variant="text" color="default" @click="clearFilters">
              Clear Filters
            </VBtn>
          </div>
        </VCardText>
      ...
      <div v-else-if="groupedSessionsKeys.length" class="schedule-list">
        <div v-for="date in groupedSessionsKeys" :key="date" class="date-group">
          <div class="date-header">
            <div class="date-header-left">
              <VIcon icon="mdi-calendar" color="primary" class="mr-2" />
              <span class="date-title">{{ formatDate(date) }}</span>
              <VChip class="class-count-chip ml-2" variant="outlined" size="small">
                {{ groupedSessions[date].length }} classes
              </VChip>
            </div>
          </div>
          <div class="sessions-grid">
            <div v-for="session in groupedSessions[date]" :key="session.id" class="session-card">
              <div class="session-content">
                <div class="session-main">
                  <div class="session-subject">
                    <span class="subject-code">{{ session.subject_code }}</span>
                    <span class="separator">—</span>
                    <span class="subject-name">{{ session.subject_name }}</span>
                  </div>
                  <div class="session-details">
                    <div class="detail-item">
                      <VIcon icon="mdi-clock-outline" size="16" class="mr-1" />
                      {{ formatTime(session.start_datetime) }} - {{ formatTime(session.end_datetime) }}
                    </div>
                    <div class="detail-item">
                      <VIcon icon="mdi-map-marker" size="16" class="mr-1" color="success" />
                      Room {{ session.room_name || 'TBA' }}
                    </div>
                    <div class="detail-item">
                      <VIcon icon="mdi-account-tie" size="16" class="mr-1" color="info" />
                      {{ session.instructor_name }}
                    </div>
                  </div>
                </div>
                <div class="session-badges">
                  <VChip size="small" :color="sessionStatusColor(session.status)" class="status-chip" label>
                    <VIcon :icon="getStatusIcon(session.status)" size="16" class="mr-1" />
                    {{ session.status }}
                  </VChip>
                  <VChip v-if="session.term_label" size="small" variant="outlined" label>
                    {{ session.term_label }}
                  </VChip>
                </div>
              </div>
      ...
      <VCard v-else class="empty-state" elevation="0">
        <div class="empty-icon">
          <VIcon icon="mdi-calendar-blank" size="48" color="primary" />
        </div>
...
const groupedSessions = computed(() => {
  ...
  scheduleStore.sessions.forEach(s => {
    ...
    if (dateRange.value && Array.isArray(dateRange.value) && dateRange.value.length > 0) {
      ...
      if (dates.length > 0) {
        if (s.dateKey < dates[0] || s.dateKey > dates[dates.length - 1]) return
      }
    }
...
const exportColumns = [
  ...
  { key: 'start_datetime', header: 'Start Time', format: 'datetime', width: 18 },
...
const exportData = computed(() => {
  ...
  return groupedSessionsKeys.value.flatMap(date => groupedSessions.value[date])
})
...
const getStatusIcon = (status) => {
  switch (status) {
    case 'planned': return 'mdi-calendar-check'
    case 'completed': return 'mdi-check-circle'
    case 'canceled': return 'mdi-close-circle'
    case 'makeup': return 'mdi-calendar-refresh'
